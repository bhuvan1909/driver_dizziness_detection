...
thresh = 0.27
modelPath = "68_face_landmarks.dat"
...
while(validFrames < dummyFrames):
    ...
    if landmarks == 0:
        ...
        if cv2.waitKey(1) & 0xFF == 27:
            ...
            soc.sendall('k'.encode('utf-8'))
            soc.sendall('h'.encode('utf-8'))
            capture.release()
            vid_writer.release()
            soc.close()
...
if __name__ == "__main__":
    ...
    while(1):
        try:
            ...
            for i in range(0, len(leftEyeIndex)):
                cv2.circle(frame, (landmarks[leftEyeIndex[i]][0], landmarks[leftEyeIndex[i]][1]), 1, (0, 255, 0), -1, lineType=cv2.LINE_AA)

            for i in range(0, len(rightEyeIndex)):
                cv2.circle(frame, (landmarks[rightEyeIndex[i]][0], landmarks[rightEyeIndex[i]][1]), 1, (0, 0, 255), -1, lineType=cv2.LINE_AA)

            if drowsy:
                cv2.putText(frame, "! ! ! Wake Up !!!!!", (70, 50), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 0, 255), 2, cv2.LINE_AA)
                soc.sendall('k'.encode('utf-8'))
                if not ALARM_ON:
                    ALARM_ON = True
                    threadStatusQ.put(not ALARM_ON)
                ...
                cv2.putText(frame, "Blinks : {}".format(blinkCount), (80,460), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255,0,0), 2, cv2.LINE_AA)
